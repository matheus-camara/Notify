...
The test project targets both supported framework targets, so the core behavior is verified on both .NET 8 and .NET 10.
...
If publishing this as NuGet, consider splitting the optional JSON/DI integration into a separate package:

- `Notify`
- `Notify.Localization.Json`

The core package can then remain extremely stable and dependency-light.
